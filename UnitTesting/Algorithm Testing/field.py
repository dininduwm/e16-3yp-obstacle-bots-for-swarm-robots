...
def getForce(robot1, robot2, dest_flag = False, k = 15, power_factor = 1, test_case_obj = None):
    ...
    if dest_flag:
        ...
        if dist < 1 and test_case_obj:
            test_case_obj.no_of_collisions += 1
            print(dist, robot1.init_pos, robot2.init_pos)
    ...
    return [force / power_factor, angle] 
...
def getResultant(robots_data, idx, power_factor, test_case_obj):
    ''' 
    ...
    for i in range(len(robots_data)):
        ...
        if i != idx:
            Forces.append(getForce(robots_data[i], robots_data[idx], False, 15, power_factor, test_case_obj))
        ...
        if x_coord < 0 or x_coord > board_width or y_coord < 0 or y_coord > board_hight:
            test_case_obj.out_of_the_board += 1
        ...
        for j in range(4):
            ...
            Forces.append([force / power_factor, angles[j]])
