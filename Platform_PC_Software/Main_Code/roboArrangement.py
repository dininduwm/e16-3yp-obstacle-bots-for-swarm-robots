...
def createStringBots(_robot):
    return str(_robot[4][0]) + '-' + str(_robot[4][1])
...
def assignBot(listOfDest, robotList):
    ...
    for dest in listOfDest:
        if (len(idleBots) == 0):
            ...
            selBot[4] = createTuple(dest) # setting the destination
            selBot[5] = False
...
def arrageBot(robotList, message):
    ...
    print('all bots', allBots)
    print('should idle', shouldIdle)
    print('should assign', shouldAssign)
    print('\n')
